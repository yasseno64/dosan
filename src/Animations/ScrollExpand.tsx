...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  const applyProgress = useCallback((p: number) => {
    ...
    // ملحوظة: منطق إخفاء/إظهار overlayRef اتشال بالكامل من هنا.
    // الـ overlay دلوقتي ظاهر طول الوقت وثابت مكانه (اتحدد من الـ className تحت).
  }, []);
  ...
  return (
    <div
      ref={rootRef}
      className={`relative h-full w-full ${useWindowScroll ? "" : "overflow-y-auto overflow-x-hidden overscroll-contain scrollbar-none [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"} ${className}`.trim()}
      style={style}
      {...rest}
    >
